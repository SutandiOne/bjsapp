...
class KlaimForm extends Component
{
    ...
    public function forcasting($tahun, $kategori)
    {
        ...
        $year3_data = Klaim::find($year2);

        if ($year2_data !== null) {

            $kategori1 = $kategori . '_jiwa';
            $kategori2 = $year2_data[$kategori . '_s'] !== null ? $kategori . '_s' : $kategori . '_jiwa';
            $kategori3 = $year3_data[$kategori . '_s2'] !== null ? $kategori . '_s2' : $kategori . '_jiwa';

            $year1_data = $year1_data[$kategori1];
            $year2_data = $year2_data[$kategori2];
            $year3_data = $year3_data[$kategori3];

            $tahap_awal = ($aph * $year1_data) + ($aph2 * $year2_data);


            $tahap_kedua = ($aph * $tahap_awal) + ($aph2 * $year3_data);
